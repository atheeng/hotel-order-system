...
//            for id
            ...
            try {
                ...
                preparedStmt.setInt(2, card.getFoodId());
            ...
            try {
                conn = DataBaseConnection.getInstance().getConnection();
                String query = "UPDATE cart SET food_id=?,item=?,unit_price=?,quantity=?,totalAmount=? where id=?";
                PreparedStatement preparedStmt = conn.prepareStatement(query);
                preparedStmt.setInt(1, card.getFoodId());
    ...
    public List<Cart> getAllTempList() {
        ...
        try {
            conn = DataBaseConnection.getInstance().getConnection();
            Statement statement = conn.createStatement();
            ResultSet rs = statement.executeQuery("SELECT id,food_id,item,unit_price,quantity,totalAmount FROM cart");
            while (rs.next()) {
                Cart product = new Cart();
                product.setId(rs.getInt(1));
                product.setFoodId(rs.getInt(2));
...
//        for orderno;
        ...
        for (Cart tem : list) {
            try {
                Connection conn = null;
                conn = DataBaseConnection.getInstance().getConnection();
                String query = "insert into order_item(order_no,food_id,qty) values(?,?,?)";
                PreparedStatement preparedStmt = conn.prepareStatement(query);
                preparedStmt.setString(1, orderNo);
                preparedStmt.setInt(2, tem.getFoodId());
        ...
        for (Cart tem : list) {
            ...
            try {
                conn = DataBaseConnection.getInstance().getConnection();
                String query = "SELECT total_quantity FROM food WHERE id=?";
                PreparedStatement preparedStmt = conn.prepareStatement(query);
                preparedStmt.setInt(1, tem.getFoodId());
                ResultSet rs = preparedStmt.executeQuery();
                while (rs.next()) {
            ...
            try {
                conn = DataBaseConnection.getInstance().getConnection();
                String query = "UPDATE food SET total_quantity=? where id=?";
                PreparedStatement preparedStmt = conn.prepareStatement(query);
        ...
        try {
            ...
            ResultSet rs = statement.executeQuery(
                    "SELECT CONCAT(u.first_name ,' ',u.last_name) AS NAME ,o.order_no,o.no_item,o.total_amount,o.order_date,"
                            + "o.status FROM orders o INNER JOIN USER u ON o.user_id=u.id " + params + " ");
            while (rs.next()) {
    ...
    public List<Map<String, String>> getOrderItemList(String orderNo) {
        ...
        try {
            conn = DataBaseConnection.getInstance().getConnection();
            String query = "SELECT oi.order_no,p.id,p.name,p.category,oi.qty,p.price,p.description FROM food p "
                    + "INNER JOIN order_item oi ON p.id=oi.food_id where oi.order_no=?";
            PreparedStatement preparedStmt = conn.prepareStatement(query);
            preparedStmt.setString(1, orderNo);
            ResultSet rs = preparedStmt.executeQuery();
            while (rs.next()) {
                ...
                orderItemMap.put("foodName", rs.getString(2) + "-" + rs.getString(3) + "(" + rs.getString(4) + ")");
    ...
    @Override
    public boolean updateStatus(String orderNo, int statusNo) {
        System.out.println("update status");
        StatusEnum statusEnum;
        if (statusNo == 0) {
            statusEnum = StatusEnum.PROGRESS;
        } else {
        ...
        try {
            ...
            PreparedStatement preparedStmt = conn.prepareStatement(query);
            preparedStmt.setString(1, statusEnum.toString());
